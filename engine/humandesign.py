...
GATE_THEMES = {
    1:'自我表達', 2:'接納',     3:'秩序',     4:'公式化',  5:'等待',    6:'摩擦',
    7:'自我角色', 8:'貢獻',     9:'聚焦',    10:'自我行為',11:'想法',   12:'謹慎',
   13:'傾聽者',  14:'力量',    15:'極端',    16:'技能',   17:'意見',   18:'校正',
   19:'接近',    20:'當下',    21:'控制',    22:'優雅',   23:'同化',   24:'合理化',
   25:'無私',    26:'自我',    27:'照顧',    28:'玩家',   29:'承諾',   30:'認可',
   31:'影響',    32:'連續性',  33:'隱私',    34:'力量',   35:'改變',   36:'危機',
   37:'友誼',    38:'反對',    39:'挑釁',    40:'分離',   41:'收縮',   42:'增長',
   43:'突破',    44:'警覺',    45:'聚集',    46:'喜悅',   47:'實現',   48:'深度',
   49:'原則',    50:'價值觀',  51:'震驚',    52:'靜止',   53:'起始',   54:'驅動',
   55:'豐盛',    56:'刺激',    57:'直覺',    58:'喜悅',   59:'親密',   60:'接受',
   61:'神秘',    62:'細節',    63:'疑惑',    64:'困惑',
}


def get_incarnation_cross(hd_data):
    """計算人類圖業力十字（化身十字）"""
    p_det = hd_data.get('personality_details', {})
    d_det = hd_data.get('design_details', {})

    p_sun_g   = p_det.get('太陽', {}).get('gate', 0)
    p_sun_l   = p_det.get('太陽', {}).get('line', 1)
    p_earth_g = p_det.get('地球', {}).get('gate', 0)
    d_sun_g   = d_det.get('太陽', {}).get('gate', 0)
    d_earth_g = d_det.get('地球', {}).get('gate', 0)

    profile_str = hd_data.get('profile', '1/3')
    try:
        p_line = int(str(profile_str).split('/')[0])
    except Exception:
        p_line = 1

    if p_line in (1, 2):
        cross_type = '右角度十字'
        cross_desc = '個人命運：專注內在發展與自我完善，影響從內而外輻射'
    elif p_line == 3:
        cross_type = '並列十字'
        cross_desc = '固定命運：透過親身試錯學習，你的故事本身就是智慧'
    else:
        cross_type = '左角度十字'
        cross_desc = '跨個人命運：天生影響周遭環境，使命在於服務更大的集體'

    return {
        'cross_type':  cross_type,
        'cross_desc':  cross_desc,
        'p_sun':       {'gate': p_sun_g,   'line': p_sun_l,  'theme': GATE_THEMES.get(p_sun_g, '')},
        'p_earth':     {'gate': p_earth_g,                   'theme': GATE_THEMES.get(p_earth_g, '')},
        'd_sun':       {'gate': d_sun_g,                     'theme': GATE_THEMES.get(d_sun_g, '')},
        'd_earth':     {'gate': d_earth_g,                   'theme': GATE_THEMES.get(d_earth_g, '')},
        'summary': (
            f"{cross_type}：{cross_desc}\n"
            f"意識太陽 閘門{p_sun_g}·{GATE_THEMES.get(p_sun_g,'')} × "
            f"意識地球 閘門{p_earth_g}·{GATE_THEMES.get(p_earth_g,'')}\n"
            f"潛意識太陽 閘門{d_sun_g}·{GATE_THEMES.get(d_sun_g,'')} × "
            f"潛意識地球 閘門{d_earth_g}·{GATE_THEMES.get(d_earth_g,'')}"
        ),
    }


CHANNEL_THEMES = {
    (1, 8):   ('靈感表達通道', '創意需要被看見才完整，你天生是藝術家或創作者'),
    (2, 14):  ('節拍回應通道', '等待被找到，方向感在回應中自然浮現'),
    (3, 60):  ('突變通道',     '在限制中突破，你是帶來基因突變的開拓者'),
    (4, 63):  ('邏輯通道',     '質疑一切，直到找到更好的答案'),
    (5, 15):  ('韻律通道',     '固定節奏帶來穩定，你體現自然流的法則'),
    (6, 59):  ('交配通道',     '打破隔閡、建立親密，選擇性的深度連結'),
    (7, 31):  ('阿爾法通道',   '天生引領者，被群體選出來帶領方向'),
    (9, 52):  ('專注通道',     '深度聚焦是你的天賦，不要被分散'),
    (10, 20): ('覺醒通道',     '在當下活出真實自我，你的存在本身就是影響力'),
    (10, 34): ('探索通道',     '行動力強且忠於原則，探索邊界的勇者'),
    (10, 57): ('完美形態通道', '直覺驅動的存活智慧，知道如何在環境中恰好安頓'),
    (11, 56): ('好奇通道',     '刺激他人思考，各種想法信手拈來'),
    (12, 22): ('開放通道',     '等待情緒的成熟時機，然後優雅地與世界互動'),
    (13, 33): ('傳道者通道',   '聆聽故事，在對的時刻分享洞見'),
    (16, 48): ('才能通道',     '反覆練習直到精通，深度技能是你的核心資產'),
    (17, 62): ('接受通道',     '用細節和邏輯說服他人，事實是你最好的武器'),
    (18, 58): ('判斷通道',     '看見不完善並修正，追求卓越是天性'),
    (19, 49): ('感性通道',     '重視原則與資源的公平分配，情感是行動的驅力'),
    (20, 34): ('魅力通道',     '薦骨接喉，力量自然流出，這是顯示生產者的核心能量'),
    (20, 57): ('腦波通道',     '直覺閃現後立即行動，慢下來反而會錯過'),
    (21, 45): ('金錢通道',     '天生管理者，你知道如何分配與掌控資源'),
    (23, 43): ('結構化通道',   '將直覺轉化為可理解的語言，帶來突破性洞見'),
    (24, 61): ('知識通道',     '沉浸思考直到靈感浮現，你是知識的深挖者'),
    (25, 51): ('開創通道',     '以震驚與衝擊喚醒他人，無私的心是你的保護'),
    (26, 44): ('傳遞通道',     '有影響力的傳遞者，擅長讓記憶與信念流傳'),
    (27, 50): ('保存通道',     '天生照顧者，維護社群的價值觀與規範'),
    (28, 38): ('搏鬥通道',     '為有意義的事物奮鬥，尋找生命值得的理由'),
    (29, 46): ('發現通道',     '全力投入帶來喜悅，你是「說yes」的人'),
    (30, 41): ('文明通道',     '情感的渴望驅動改變，你體現人類對體驗的渴求'),
    (32, 54): ('轉化通道',     '野心與本能驅動系統演化，你是長線的戰略家'),
    (34, 57): ('力量通道',     '本能力量與直覺即時反應，強大而安靜的存在'),
    (35, 36): ('短暫通道',     '渴望一切體驗，情感起伏是成長的材料'),
    (37, 40): ('社群通道',     '情感的交易：我給你支持，你給我空間與認可'),
    (39, 55): ('情緒通道',     '在豐盛與匱乏之間搖擺，尋找靈性的生命意義'),
    (42, 53): ('成熟通道',     '啟動週期並走完全程，開始與結束的力量'),
    (47, 64): ('抽象通道',     '反思過去的體驗，將混沌提煉為智慧'),
}


OPEN_CENTER_DESC = {
    '頭腦': {
        'challenge': '容易為不屬於自己的問題感到焦慮，習慣被外界壓力驅動思考',
        'gift':      '思維開放，能感知並放大各種靈感，是絕佳的智識橋樑',
        'tip':       '允許想法流過而不強迫找到答案，你不需要解決所有問題',
    },
    '邏輯': {
        'challenge': '容易受他人思維影響，難以辨別哪些想法真正屬於自己',
        'gift':      '彈性評估各種概念，思維適應力極高',
        'tip':       '在安靜環境中做重要決定，不要讓外來聲音淹沒內在判斷',
    },
    '喉嚨': {
        'challenge': '容易在錯誤時機發言或為引起注意而說話，感到表達困難',
        'gift':      '聲音品質因人因地而異，表達適應力強',
        'tip':       '等待被邀請發言，真正的影響力來自對的時機而非音量',
    },
    'G中心': {
        'challenge': '方向感和身份認同不固定，容易在關係中迷失自我',
        'gift':      '能融入不同環境和人群，像鏡子般反映周圍的方向',
        'tip':       '待在充滿愛的人身邊、喜歡的空間裡，方向自然浮現',
    },
    '心輪': {
        'challenge': '感到需要不斷「證明自己」，容易過度承諾導致精疲力竭',
        'gift':      '不被固定意志力束縛，能靈活運用能量',
        'tip':       '停止用意志力強迫自己，你的價值無需被證明，練習說「不」',
    },
    '情緒': {
        'challenge': '容易被他人情緒淹沒，在情緒高張環境中做出錯誤決定',
        'gift':      '極度敏銳地感知他人情感，天然的同理心',
        'tip':       '先讓自身情緒場穩定後再做決定，不把他人情緒當成自己的',
    },
    '薦骨': {
        'challenge': '不易感知「夠了」的信號，可能過度工作或能量分散',
        'gift':      '可調整能量輸出節奏，適應不同頻率的人與環境',
        'tip':       '投射者/顯示者需特別注意疲憊信號，充分休息才有持續力',
    },
    '脾/直覺': {
        'challenge': '容易忽略身體健康和安全信號，對有害情境缺乏即時警覺',
        'gift':      '能感知並放大他人健康訊號，是直覺的天然擴音器',
        'tip':       '定期做身體掃描，留意那些微弱的「感覺不對」直覺',
    },
    '根部': {
        'challenge': '容易感到需要快速完成事情來解除壓力，導致衝動決策',
        'gift':      '不受固定壓力驅動，可在各種節奏中靈活運作',
        'tip':       '感到急迫衝動時先暫停，問自己「這真的重要嗎？」',
    },
}


def get_open_center_analysis(hd_data):
    """分析開放中心的制約模式與成長機會"""
    defined = set(hd_data.get('defined_centers', []))
    result = []
    for center, desc in OPEN_CENTER_DESC.items():
...
def get_active_channel_themes(hd_data):
    """返回激活通道的主題列表"""
    active = hd_data.get('active_channels', [])
    themes = []
    for ch in active:
        key = tuple(sorted(ch))
        if key in CHANNEL_THEMES:
            name, desc = CHANNEL_THEMES[key]
            themes.append({'gates': list(key), 'name': name, 'desc': desc})
    return themes
